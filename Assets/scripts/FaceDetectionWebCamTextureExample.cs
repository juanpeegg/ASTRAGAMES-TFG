...
namespace OpenCVForUnityExample
{
    ...
    public class FaceDetectionWebCamTextureExample : MonoBehaviour
    {
        ...
        /// </summary>
        protected static readonly string LBP_CASCADE_FILENAME = "objdetect/haarcascade_frontalface_alt.xml";

        public Vector2 centroRectangulo;
        public int lado;
        ...
        void Update()
        {
            if (webCamTextureToMatHelper.IsPlaying() && webCamTextureToMatHelper.DidUpdateThisFrame())
            {
                ...
                for (int i = 0; i < rects.Length; i++)
                {
                    ...
                    Imgproc.rectangle(rgbaMat, new Point(rects[i].x, rects[i].y), new Point(rects[i].x + rects[i].width, rects[i].y + rects[i].height), new Scalar(255, 255, 255, 255), 2);
                }
                
                if(rects.Length != 0)
                {
                    lado = rects[0].width/2;
                    centroRectangulo = new Vector2(rects[0].x + rects[0].width/2, rects[0].y + rects[0].height/2);
                    //Debug.Log("Coordenada x: " + rects[0].x + ", coordenada y: " + rects[0].y + ", W: " + rects[0].width + ", H: " + rects[0].height);
                }
