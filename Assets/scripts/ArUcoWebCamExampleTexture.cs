...
namespace OpenCVForUnityExample
{
    ...
    public class ArUcoWebCamExampleTexture : MonoBehaviour
    {
        ...
        //Lleva las coordenadas en pantalla del primer marcador detectado
        public Vector3 coordenadas = Vector3.zero;
        //Lleva las coordenadas en pantalla del primer marcador detectado
        public Vector2 coordenadasPantalla = Vector2.zero;
        ...
        void Start()
        {
            ...
            //Debug.Log("ejecuta esto " + webCamTextureToMatHelper);
        ...
        public void OnWebCamTextureToMatHelperInitialized()
        {
            ...
            Mat webCamTextureMatInvertida = webCamTextureToMatHelper.GetMat(); 

            Core.flip(webCamTextureMat, webCamTextureMatInvertida, 1);

            texture = new Texture2D(webCamTextureMatInvertida.cols(), webCamTextureMatInvertida.rows(), TextureFormat.RGBA32, false);
            Utils.matToTexture2D(webCamTextureMatInvertida, texture);
            ...
            // Usamos el diccionario predefiniido
            ...
            if (webCamTextureToMatHelper.IsPlaying() && webCamTextureToMatHelper.DidUpdateThisFrame())
            {
                ...
                switch (markerType)
                {
                    ...
                    case MarkerType.CanonicalMarker:
                        // Comparo la lista de detectados con la lista anterior. Si es la misma, lo dejo todo igual. Si hay diferencias, los que no están los quito.
                        List<int> idents = new List<int>();
                        for (int i=0; i< ids.total(); i++)
                        {
                            idents.Add((int)ids.get(i,0)[0]);
                        }

                        List<int> diferencia = identificadores.Except(idents).ToList();

                        //Los que no están, los pongo a falso
                        foreach(int n in diferencia)
                        {
                            arGameObject[n].SetActive(false);
                        }

                        //Actualizo la lista para poder comprobar en el siguiente frame
                        identificadores = idents;

                        // If at least one marker detected
                        if (ids.total() > 0)
                        {
                            //Se pintan los bordes de los marcador aruco que se ha encontrado
                            Objdetect.drawDetectedMarkers(rgbMat, corners, ids, new Scalar(0, 255, 0));

                            //Esto estará activado por defecto
                ...
                Core.flip(rgbaMat, webCamTextureMatInvertida, 1);

                Vector2 coordsInvertidas = new Vector2(webCamTextureToMatHelper.requestedWidth - coordenadasPantalla.x,coordenadasPantalla.y);

                Point centroCirculo = new Point(coordsInvertidas.x, coordsInvertidas.y);

                Imgproc.circle(webCamTextureMatInvertida, centroCirculo, 5, new Scalar(0, 255, 0), -1);
        ...
        private void EstimatePoseCanonicalMarker(Mat rgbMat)
        {
            ...
            {
                //Para cada uno de los marcadores detectados;
                for (int i = 0; i < ids.total(); i++)
                {
                    ...
                    using (MatOfPoint2f imagePoints = new MatOfPoint2f(corner_4x1))
                    {
                        ...
                        // This example will display all the ARObject which correspond to detected markers.
                        //Debug.Log("El identificador es:" + ids.get(0,0)[0]);
                        UpdateARObjectTransform(rvec, tvec, (int)ids.get(i, 0)[0]);
                        
                        //Solo lo hago con el primero detectado
                        if (i == 0)
                        {
                            Vector2 cornerESI = new Vector2((float)corners[0].get(0, 0)[0],(float) corners[0].get(0, 0)[1]);
                            Vector2 cornerESD = new Vector2((float)corners[0].get(0, 2)[0], (float)corners[0].get(0, 2)[1]);
                            Vector2 cornerEII = new Vector2((float)corners[0].get(0, 1)[0], (float)corners[0].get(0, 1)[1]);
                            
                            coordenadas = new Vector3(((cornerESI + cornerESD)/2).x,
                                ((cornerESI + cornerEII) / 2).y, 0);

                            int largo = webCamTextureToMatHelper.requestedWidth / 2; //320
                            int alto = webCamTextureToMatHelper.requestedHeight / 2; //240

                            coordenadasPantalla.x = coordenadas.x;
                            coordenadasPantalla.y = coordenadas.y;

                            coordenadas.x = coordenadas.x - 320;
                            coordenadas.y = 240 - coordenadas.y;
        ...
        */
        ...
        //Colocamos el objeto que sale con el aruco en la posición donde se encuentra el aruco
        private void UpdateARObjectTransform(Mat rvec, Mat tvec, int id)
        {
            ...
            //tvec.get(0, 0)[2];
            //Debug.Log("La distancia de la cámara al identificador " + id + " es " + tvecArr[2]);
            ...
            //esto es negativo por defecto
            if (shouldMoveARCamera)
            {

                ARM = arGameObject[id].transform.localToWorldMatrix * ARM.inverse;
            ...
            else
            {
                ...
                //Ahora se pone con la orientacion de como se ha encontrado en la cámara               
                ARUtils.SetTransformFromMatrix(arGameObject[id].transform, ref ARM);
                arGameObject[id].SetActive(true);
            }
        }
        ...
        private void ResetObjectTransform()
        {
            ...
            //Ponemos todos en su sitio inicial de nuevo
            foreach(GameObject go in arGameObject)
            {
                ARUtils.SetTransformFromMatrix(go.transform, ref i);
            }
